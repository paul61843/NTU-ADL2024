...
def main():
    ...
    label_column_name = "relevant" if "relevant" in column_names else "relevant"
    ...
    def preprocess_function(examples):
        context_dict = load_dataset('json', data_files="./context.json")["train"]



        first_sentences = [[context] * 4 for context in examples[context_name]]
        second_sentences = [
            [context_dict[pid]["text"] for pid in paragraph_ids]
            for paragraph_ids in examples['paragraphs']
        ]
        
        labels = [
            paragraph_ids.index(relevant_id)
            for paragraph_ids, relevant_id in zip(examples["paragraphs"], examples["relevant"])
        ]

        test = [context_dict[paragraph] for paragraph in examples["paragraphs"]]
